...
void main() {
  ...
  late LocalDataSourceUseCase useCase;
  setUp(() {
    mapper = MeteoMapper();
    mockSharedPreferences = MockSharedPreferences();
    repo = DataLocalRepoIMPL(mapper: mapper, shared: mockSharedPreferences);
    useCase = LocalDataSourceUseCase(aRepo: repo);
  });

  group('LocalDataSourceUseCase testing group', () {
    test('loadFromCache foo test should return Right MeteoEntity', () async {
      ...
      final result = await useCase.loadFromCache(cityName: cityName);
      result.fold(
        (l) {
          print(l.message);
          fail('wrong answer!');
